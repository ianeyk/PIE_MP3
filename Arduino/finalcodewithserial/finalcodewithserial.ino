...
// Select motor controller ports M1 and M2
...
float ratio;      // set up wheel speed ratio
float RATIOc = 4; // initial ratio of wheel speeds during turn, controlling turning radius
...
void setup()
{
  ...
  printColumns(); // create column headers for CSV output
...
void turnLeft(int speed) // turn robot left; because leftSpeed and rightSpeed are both positive, the robot will also move forward
{
...
void turnRight(int speed) // turn the robot right; because leftSpeed and rightSpeed are both positive, the robot will also move forward
{
  float leftSpeed = RATIOc * speed;   // set the left motor speed to the ratio times speed (left motor will go faster than right)
...
{                                  // test if left sensor is on the tape or not
  return sensorValues[0] > THRESH; // if the left sensor value is greater than the threshold, then it is on the tape
}

bool rightSensorOnTape()
{                                  // test if right sensor is on the tape or not
  return sensorValues[1] > THRESH; // if the right sensor value is greater than the threshold, then it is on the tape
}

void printColumns()
{ // set up headers for printed data
  Serial.print("Time");
  Serial.print(", ");
...
void printData()
{ // function to print information about timer, sensor values, and motor values
  Serial.print(millis());
  Serial.print(", ");
  ...
  Serial.print(", ");
  Serial.print(motorValues[0]);
  Serial.print(", ");
  Serial.print(motorValues[1]);
  Serial.println("");
}
...
void loop()
{
  ...
  // the serial commands are input as a 10-digit string of the form SSSDDDDRRR
  // where the first three digits represent the base speed;
  // the next 4 digits represent the delay;
  // and the last three represent the ratio between the wheels when turning, in hundredths
  ...
  readSensors(); // read the sensor data and store it in sensorValues

  if (leftSensorOnTape() || rightSensorOnTape())
  {                   // if the left or right sensors are on the tape
